...
class MapMatchingNDT{
	private:
		...
		/*viewer*/
		pcl::visualization::PCLVisualizer viewer{"map_matching_ndt"};
		/*cloud*/
...
MapMatchingNDT::MapMatchingNDT()
	:nhPrivate("~")
{
	sub_pc = nh.subscribe("/velodyne_points", 1, &MapMatchingNDT::CallbackPC, this);
	sub_pose = nh.subscribe("/ekf/pose", 1, &MapMatchingNDT::CallbackPose, this);
	...
	nhPrivate.param("filename", filename, std::string("hoge.pcd"));
...
void MapMatchingNDT::Load(void)
{
	ros::Time t_start = ros::Time::now();
	std::cout << "Loading ..." << std::endl;
	if(pcl::io::loadPCDFile<pcl::PointXYZ>(filename, *pc_map) == -1){
		std::cout << "Loading error: " << filename << std::endl;
		exit(1);
	}
	Downsampling(pc_map, leafsize_source);
	std::cout << "Done ( " << (ros::Time::now() - t_start).toSec() << " [s])" << std::endl;
}

void MapMatchingNDT::CallbackPose(const geometry_msgs::PoseStampedConstPtr& msg)
{
	pose_ekf = *msg;
	first_callback_pose = false;
}

void MapMatchingNDT::CallbackPC(const sensor_msgs::PointCloud2ConstPtr& msg)
{
...
bool MapMatchingNDT::Transformation(void)
{
	...
	std::cout << "downsampling clock [s] = " << ros::Time::now().toSec() - t_start << std::endl;
	/*drop out*/
	...
	std::cout << "aligning clock [s] = " << ros::Time::now().toSec() - t_start << std::endl;
	/*drop out*/
	...
	std::cout << "transformation clock [s] = " << ros::Time::now().toSec() - t_start << std::endl;

	return true;
}

void MapMatchingNDT::PassThroughFilter(pcl::PointCloud<pcl::PointXYZ>::Ptr pc_in, pcl::PointCloud<pcl::PointXYZ>::Ptr pc_out, std::vector<double> range)
{
...
void MapMatchingNDT::Downsampling(pcl::PointCloud<pcl::PointXYZ>::Ptr pc, double leafsize)
{
...
/* void MapMatchingNDT::ApproximateDownsampling(pcl::PointCloud<pcl::PointXYZ>::Ptr pc, double leafsize) */
/* { */
...
void MapMatchingNDT::Visualization(void)
{
...
void MapMatchingNDT::Publication(void)
{
	/*publish*/
...
Eigen::Quaternionf MapMatchingNDT::QuatMsgToEigen(geometry_msgs::Quaternion q_msg)
{
...
geometry_msgs::Quaternion MapMatchingNDT::QuatEigenToMsg(Eigen::Quaternionf q_eigen)
{
...
{
	ros::init(argc, argv, "map_matching_ndt");
	
	MapMatchingNDT map_matching_ndt;
	map_matching_ndt.Load();
